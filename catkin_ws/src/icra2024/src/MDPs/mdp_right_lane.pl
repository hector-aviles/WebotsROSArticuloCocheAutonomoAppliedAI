...
utility(success(1), 1) :- not(rearEnd_crash(0)), not(sideSwipe_crash(0)).
utility(f_N(1), 0.2). 
...
rearEnd_crash(1) :- not(f_N(0)), cruise, not(keep_distance), not(change_lane).
rearEnd_crash(1) :- not(f_NW(0)), change_lane.
sideSwipe_crash(1) :- not(sW1(0)), not(sW1(0)), change_lane.
sideSwipe_crash(1) :- not(f_W(0)), change_lane.
